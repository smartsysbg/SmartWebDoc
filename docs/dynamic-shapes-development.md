...
The shape creation process begins with the `Create Shape` button. This action allows the user to provide a name for the new shape and will create a new shape file upon pressing `Save`.

![](./media/dynamic-shapes-development/2.png)

The newly created shape file will be available in the `UnApproved` folder in the `Media Library`, along with other recently created shapes `(7)`.
...
Basic `svg` elements that are intended to have dynamic behavior are assigned the  
...
Different shapes are built for different purposes. They represent different data and have different behaviors. The type of the shape is determined by the data that it has to represent. In this section we will go through the process of creating a shape with the help of an example. 
...
!!!error "Theme Support:"
     ...
     `i-ng-class` `directive`.

     ![](./media/dynamic-shapes-development/6.png)

     The `ApplyNgTheme()` function takes one parameter which is the name of the class
     that we want our `svg` element to have.
...
!!!error "Important:"
     Shapes can't contain other shapes. This means that after the elements have been grouped once, that group can't be grouped again with a new element. Grouping should only be done after all of the required elements have been created/added. If you forget to group the elements, it will be done automatically after pressing the `Save` button.

!!!error "Important"
     When there are two or more `base components` with the same `Item Substitution` present. Their `Item Substitution` fields should be set before grouping. This field is unique for each  
     `base component`. Two components must not have the same `Item Substitution`.
...
The configuration will determine the features that the shape provides as well as the behavior that will represent data changes. The shape has its own `Item Configuration` panel, comprised of three different tabs.
...
* `name` `(3)` - provides a name for the shape (will not be shown in this chapter)

![](./media/dynamic-shapes-development/11.png)

The `ng-init` `directive` initializes the point parameters, which the shape will represent, as variables that can be used by the `base components` and the shape-specific script.

The image below represents the custom parameters for our example data acquisition shape. Each row consists of two columns. The first column contains the variable name, while the second column contains the point parameter, the value of which will be used by the `base components` and the shape script. In this case, the first four variables `(1)` will be used by the `base components` directly. They will not be used in our script because the `base components` have built-in behavior which will represent the data without our interference. The other three variables `(2)` will be used in our script, because they will allow our shape to have additional behavior that is not handled by a `base component`.

![](./media/dynamic-shapes-development/12.png)

The following image represents the custom parameters after they have been added to the specific components that require them. The parameters are specified in a specific way (syntax). They must be surrounded by curly brackets { } and a percent sign % - {%parameter%}. Assigning a point parameter to a custom parameter (`ng-init` variable) also provides flexibility in cases where multiple components use the same type of point parameter. Changes to the custom parameter in the `Directives Tab` will result in changes in the `base components`. 
...
Calling the shape-specific script is the last step in the configuration process. It enables the user to implement custom behavior and and functionality for the shape by calling a custom *JavaScript* function. The function call itself is done with the help of specific `base components` or with the help of specific `directives`.

!!! note "Function calls through `directives`:"
     Calling a function through a `directive` is usually done in cases that require the function to be executed only once. This approach is relevant for shape types such as Navigations or Matrixes etc. Standard shapes represent data on an *on-update* basis, which demands the use of `base components`.

There are three `base components` that allow functions to be called:

* `Alphanumeric`
* `Parameter Container` 
* `Indicator Bar`

Each of these components contains a `Function` field that will call the custom function when there is new data (on an *on-update* event). The field has a specific syntax that begins with square  
brackets _[ ]_. The data between these brackets is in *JSON* format. The first property (_action_) defines the action that, when triggered, will call the function. The second property (_function_) defines the name of the function that will be called.

!!!error "Important:"
     Only one custom function should exist per shape, and it should have the same name as the shape, followed by *_OnUpdate*. 
     
     Example: *Scd_DataAcq_Example_OnUpdate*.

     The custom function can be called from any `Alphanumeric`, `Parameter Container`, or `Indicator Bar`. Even if there are multiple components of the same type. When an *on-update* event occurs - each  
     `base component` is updated. It is good practice to call the function just once per *on-update* event (the function should **not** be called by each component). 
...
Custom functions are a vital part of the shape creation process. They determine the way the data is processed and presented. Essentially, they determine the behavior of the shape. 
...
Individual shape behavior is based on the shape configuration (shape parameters).
...
Accessing different `svg` elements is vital for providing dynamic behavior for the shape. The following function will return a specified element, which can then be altered by the code:
```
...
The scope that the shape script has access to is the scope of the component that it was called from. In some cases, scopes of other components need to be accessed in order to achieve the desired shape behavior. To do so, the following *SmartWEB* function can be used:
...
Additionally, *jQuery* can be used:
```
...
A slightly different approach is used when theming is required. The most common way to manipulate a `base element` that supports theming is to use a variable that is added as a parameter for the `ApplyNgTheme()` function in the `i-ng-init` directive. 
...
As a final example for this chapter, we will create the custom function for our example data acquisition shape. This is a production-grade shape used at a professional level. Most of the functionality in this case is handled by the built-in code of the `base components`. Formatting the value and providing an alarm frame for the shape is the only additional behavior required.
